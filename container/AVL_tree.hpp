#ifndef HWSHQTB__CONTAINER__AVL_TREE_HPP
#define HWSHQTB__CONTAINER__AVL_TREE_HPP
...
namespace hwshqtb {
    ...
    namespace container {
        ...
        struct AVL_tree_trait<std::pair<const Key, Mapped>> {
            using key_type = Key;
        ...
        struct AVL_tree_node {
            ...
            AVL_tree_node* less = nullptr;
            AVL_tree_node* greater = nullptr;

            template <typename... Ts>
            HWSHQTB_CONSTEXPR20 AVL_tree_node(Allocator& allocator, Ts&&... values):
            ...
            HWSHQTB_CONSTEXPR20 std::size_t left_height()const {
                return this->left == nullptr ? 0 : this->left->height;
            }
            HWSHQTB_CONSTEXPR20 std::size_t right_height()const {
                return this->right == nullptr ? 0 : this->right->height;
            }
            HWSHQTB_CONSTEXPR20 void update_height() {
        ...
        template <typename Value, class Allocator>
        class AVL_tree_iterator {
            friend AVL_tree_const_iterator<Value, Allocator>;
            template <typename, typename, typename, bool>
            friend class AVL_tree;

        private:
            HWSHQTB_CONSTEXPR20 AVL_tree_iterator(AVL_tree_node<Value, Allocator>* node, bool end):
                _node(node), _end(end) {};
        ...
        public:
            ...
            HWSHQTB_CONSTEXPR20 AVL_tree_iterator& operator++() {
            ...
            HWSHQTB_CONSTEXPR20 AVL_tree_iterator operator++(int)const {
                AVL_tree_iterator old = *this;
                ++*this;
                return old;
            }
            HWSHQTB_CONSTEXPR20 AVL_tree_iterator operator--(int)const {
            ...
            HWSHQTB_CONSTEXPR20 bool operator==(const AVL_tree_iterator& other)const noexcept {
        ...
        private:
            AVL_tree_node<Value, Allocator>* _node;
            bool _end;
        ...
        private:
            HWSHQTB_CONSTEXPR20 AVL_tree_const_iterator(AVL_tree_node<Value, Allocator>* node, bool end):
                _node(node), _end(end) {};
        ...
        public:
            ...
            HWSHQTB_CONSTEXPR20 AVL_tree_const_iterator(AVL_tree_iterator<Value, Allocator> iter):
            ...
            HWSHQTB_CONSTEXPR20 AVL_tree_const_iterator& operator++() {
                if (_node == nullptr) return *this;
                if (_end) return *this;
                if (_node->greater == nullptr) {
                    _end = true;
            ...
            HWSHQTB_CONSTEXPR20 AVL_tree_const_iterator operator++(int)const {
                AVL_tree_iterator old = *this;
                ++*this;
                return old;
            }
            ...
            HWSHQTB_CONSTEXPR20 bool operator==(const AVL_tree_const_iterator& other)const noexcept {
                return _end == other._end && _node == other._node;
            }
            HWSHQTB_CONSTEXPR20 bool operator!=(const AVL_tree_const_iterator& other)const noexcept {
                return _end != other._end || _node != other._node;
        ...
        template <typename Value, class Compare, class Allocator, bool Multiple>
        class AVL_tree {
            using tree_node = AVL_tree_node<Value, Allocator>;

        public:
            using key_type = typename AVL_tree_trait<Value>::key_type;
            using mapped_type = typename AVL_tree_trait<Value>::mapped_type;
            using value_type = typename AVL_tree_trait<Value>::value_type;
        ...
        public:
            ...
            using iterator = std::conditional_t<AVL_tree_trait<Value>::is_set, AVL_tree_const_iterator<Value, Allocator>, AVL_tree_iterator<Value, Allocator>>;
            using const_iterator = AVL_tree_const_iterator<Value, Allocator>;
            using reverse_iterator = std::reverse_iterator<iterator>;
            using const_reverse_iterator = std::reverse_iterator<const_iterator>;
            using insert_return_type = std::pair<iterator, bool>;

        public:
            constexpr AVL_tree(const Compare& compare = Compare(), const Allocator& allocator = Allocator())noexcept(std::is_nothrow_default_constructible_v<Compare>&& std::is_nothrow_default_constructible_v<Allocator>):
                _root(nullptr), _begin(nullptr), _end(nullptr), _compare(compare), _allocator(allocator), _tree_node_allocator(allocator) {}
            /*constexpr AVL_tree(std::initializer_list<value_type> vs, const Compare& compare = Compare(), const Allocator& allocator = Allocator()) :
            ...
            iterator erase(const_iterator pos) {
                ...
                balance(node->parent);
                destory_node(node);
                if (_root == node) _root = _begin = _end = nullptr;
                return result;
            }
        ...
        private:
            ...
            constexpr void balance(tree_node* now) {
                while (now) {
                    if (now->left_height() >= now->right_height() + 2) {
                        tree_node* son = now->left;
                        if (son->left_height() >= son->right_height()) {
                            now->left = son->right;
                            if (son->right) son->right->parent = now;

                            son->parent = now->parent;
                            if (now->parent) (now->parent->left == now ? now->parent->left : now->parent->right) = son;
                            ...
                            tree_node* sson = son->right;
                            now->left = sson->right;
                            if (sson->right) sson->right->parent = now;

                            son->right = sson->left;
                            if (sson->left) sson->left->parent = son;

                            sson->parent = now->parent;
                            if (now->parent) (now->parent->left == now ? now->parent->left : now->parent->right) = sson;
                    ...
                    else if (now->right_height() >= now->left_height() + 2) {
                        tree_node* son = now->right;
                        if (son->right_height() >= son->left_height()) {
                            now->right = son->left;
                            if (son->left) son->left->parent = now;
                            ...
                            tree_node* sson = son->left;
                            now->right = sson->left;
                            if (sson->left) sson->left->parent = now;

                            son->left = sson->right;
                            if (sson->right) sson->right->parent = son;
